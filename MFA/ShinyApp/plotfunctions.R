...
plot_eigen <- function(x){
  ...
  p2<-ggplot2::ggplot(data=eigen,ggplot2::aes(x=factor(id,levels=id),y=eigen,fill=factor(id,levels=id)))+
    ...
    ggplot2::theme(plot.title = ggplot2::element_text(size=22, face="bold",vjust=1,color="grey40"),
                   ...
                   panel.grid.minor = ggplot2::element_line(colour = "grey90",size=0.4),
                   panel.grid.major = ggplot2::element_line(colour = "grey90",size=0.4),
    ...
    ggplot2::guides(fill = ggplot2::guide_legend(keywidth = 1.8, keyheight = 1.8))
    p2
...
plot_common <- function(x,dim){
  ...
  p1<-ggplot2::ggplot()+
    ggplot2::geom_point(data=compromise,ggplot2::aes(x=compromise[,1],y=compromise[,2],color=id),size=6)+
    ggplot2::theme(plot.title = ggplot2::element_text(size=22, face="bold",vjust=1,color="grey50"),
                   axis.title.x = ggplot2::element_text(size=18, face="bold",vjust=-0.5,color="grey50"),
                   axis.title.y = ggplot2::element_text(size=18, face="bold",vjust=0.5,color="grey50"),
                   legend.title=ggplot2::element_blank(),
                   panel.background = ggplot2::element_blank(),
                   panel.grid.minor = ggplot2::element_line(colour = "grey90",size=0.4),
                   panel.grid.major = ggplot2::element_line(colour = "grey90",size=0.4),
    ...
    ggplot2::guides(color = ggplot2::guide_legend(keywidth = 1.8, keyheight = 1.8))+
    # plot x and y axis
    ggplot2::annotate("segment", x=-Inf,xend=Inf,y=0,yend=0,arrow=ggplot2::arrow(length=ggplot2::unit(0.3,"cm")),size=0.8,color="grey60") +
    ggplot2::annotate("segment", y=-Inf,yend=Inf,x=0,xend=0,arrow=ggplot2::arrow(length=ggplot2::unit(0.3,"cm")),size=0.8,color="grey60")+
    # center (0,0)
...
plot_loadings <- function(x,dim){
  ...
  p4<-ggplot2::ggplot()+
    ggplot2::geom_point(data=loadings,ggplot2::aes(x=loadings[,1],y=loadings[,2],color=id),size=3)+
