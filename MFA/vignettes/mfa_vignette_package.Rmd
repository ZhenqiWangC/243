...
### 2.9 Bootstrap for Factor Scores
...
```{r, out.width = '720px', out.length = '700px',dpi=2000,fig.asp=0.8,fig.width=9,fig.length=5,fig.align='center',message=F,results='hide'}
plot(mfa_wine,dim=c(1,2),singleoutput=NULL)
```
```{r, out.width = '500px', out.length = '500px',dpi=2000,fig.width=6,fig.asp=0.8,fig.align='center',message=F,results='hide'}
plot(mfa_wine,dim=c(1,2),singleoutput='eig')
```
```{r, out.width = '500px', out.length = '500px',dpi=2000,fig.width=6,fig.asp=0.8,fig.align='center',message=FALSE,results='hide'}
...
`RV()` takes two groups of data and return the $R_v$ Coefficient between them. To see detailed explanation of the RV coeffecient, see [2.8 $R_v$ Coefficient and $L_g$ Coefficient](#anchor_co).
The usage of `RV()` is:
```{r,eval=FALSE}
RV(table1,table2)
```
